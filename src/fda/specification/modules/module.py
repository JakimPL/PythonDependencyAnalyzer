...
from fda.exceptions import FDAInvalidOriginTypeError, FDAMissingModuleNameError, FDAPathResolutionError
...
class Module(Specification):
    ...
    def validate_module(self) -> Self:
        if not self.name:
            raise FDAMissingModuleNameError("Module name cannot be empty")

        if self.package is not None and not self.package:
            raise FDAMissingModuleNameError("Package name cannot be empty if provided")

        if self.origin is not None:
            validate_spec(self.spec, validate_origin=True)
            if self.origin_type != OriginType.PYTHON:
                raise FDAInvalidOriginTypeError(
                    f"Module '{self.name}' has invalid origin type '{self.origin_type}' for file origin"
                )
        elif self.origin_type == OriginType.PYTHON:
            raise FDAInvalidOriginTypeError(f"Module '{self.name}' has file origin type but no origin path")
