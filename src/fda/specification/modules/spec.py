...
def validate_spec_origin(spec: ModuleSpec) -> None:
    ...
    if spec.origin in SPECIAL_ORIGINS:
        raise FDAFrozenOriginError(f"Module '{name}' is frozen/built-in and cannot be analyzed")

    origin = Path(spec.origin)
    if not origin.is_absolute():
        raise FDARelativeBasePathError(f"Module '{name}' has non-absolute origin path: '{spec.origin}'")
    ...
    if origin.is_file() and origin.suffix != ".py":
        raise FDAInvalidOriginTypeError(f"Module '{name}' has non-Python origin file: '{spec.origin}'")
